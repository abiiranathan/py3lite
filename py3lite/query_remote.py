...
class BaseConnection(object):
    ...
    def __init__(self, model_instance):
        ...
        self._name = model_instance.__class__.__name__
...
class Query(BaseConnection):
    ...
    def build(self):
        ...
        """
        self.validate_table()
        self.value = self.value.format(', '.join(self._value_list).rstrip(', '), self.table)
    ...
    def validate_table(self):
        ...
        if not hasattr(self, 'table'):
            raise ValueError(f"Set a table by calling with_table on {self.__class__}")
    ...
    def offset(self, count):
        if self._limit is None:
            raise TypeError(f"Can't set offset on {self.__class__} object without specifying limit!")
        self._offset = count
        return self
    ...
    def paginate_by(self, count):
        ...
        if self.orderby_condition == '':
            raise TypeError("Pagination requires you to set order on your query")
    ...
    def fetchall(self):
        ...
        if self.bound_params:
            return getattr(self.model.proxy, f'remote_execute_data__{self._name}')(query, self.bound_params, 'all')
        else:
            return getattr(self.model.proxy, f'remote_execute__{self._name}')(query, 'all')

    def fetchone(self):
        query = self.build()
        if self.bound_params:
            return getattr(self.model.proxy, f'remote_execute_data__{self._name}')(query, self.bound_params, 'one')
        else:
            return getattr(self.model.proxy, f'remote_execute__{self._name}')(query, 'one')

    def fetchmany(self, n):
        query = self.build()
        if self.bound_params:
            return getattr(self.model.proxy, f'remote_execute_data__{self._name}')(query, self.bound_params, n)
        else:
            return getattr(self.model.proxy, f'remote_execute__{self._name}')(query, n)
    ...
    def fetch(self, how_many):
        ...
        return self.fetchmany(how_many)
    ...
    def as_view(self, view_name):
        ...
        query = self.build()
        return getattr(self.model.proxy, f'remote_execute__{self._name}')(query)

    def __add__(self, other):
        """Add two Query objects to create a UNION query"""
        assert isinstance(other, Query), f'{other} should be of the same type as {self}'
...
class UnionQuery:
    ...
    def execute(self, sql=None):
        return getattr(self.model.proxy, f'remote_execute__{self._name}')(sql or self.sql)
...
class InsertQuery(BaseConnection):
    def save(self):
        ...
        self.model.pre_save()
        result = getattr(self.model.proxy, f'remote_save__{self._name}')(query, data)
...
class UpdateQuery(BaseConnection):
    ...
    def save(self, **kwargs):
        ...
        if not hasattr(self.model, 'id'):
            raise ValueError(f"No prmary key id field in instance {self.model}")
        ...
        # Add pre_save signals
        self.model.pre_update()

        rowcount = getattr(self.model.proxy, f'remote_update__{self._name}')(query, data)
        # Run post_save signals
...
class DeleteQuery(BaseConnection):
    def delete(self, **kwargs):
        ...
        rowcount = getattr(self.model.proxy, f'remote_delete__{self._name}')(query, data)
